...
function wordCountEngine(document) {
    ...
    for(let i = 0; i < docArr.length; i++){
      docArr[i] = docArr[i].toLowerCase();
      let newStr = ""
      for (let j = 0; j < docArr[i].length; j++){           
        if(regex.test(docArr[i][j]) == true && docArr[i][j] != " "){
          newStr += docArr[i][j];
        }        
      }
      if(hashT[newStr] == undefined && newStr.length > 0){
        hashT[newStr] = 1;
      } else if (newStr.length > 0) {
        hashT[newStr] ++;
        if(hashT[newStr]>maxCounter){maxCounter = hashT[newStr]}
      }
    }
