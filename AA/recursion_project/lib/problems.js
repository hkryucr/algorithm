...
function fileFinder(directories, targetFile) {
    ...
    //     if(key[0] ==='/') 
    // }
    let topKeys = Object.keys(directories); //  ['/images', '/music']    desktop['/images'] = ['app_academy_logo.svg', '/park', '/pets']
    for (let i = 0; i < topKeys.length; i++){
        // const curKey = topkey[i];
        if(topKeys[i][0] === "/"){
            let recurFildFinder = fileFinder(directories[topKeys[i]], targetFile);
            if (recurFildFinder) return true;
        } else {
            if(topKeys[i] === targetFile) return true;
        }
    }
    return false;
}
...
function pathFinder(directories, targetFile) {
    for(let key in directories){
        ...
        if(key[0] === '/'){
            let prevPathFinder = pathFinder(directories[key], targetFile); // 'everlong.flac
            if(prevPathFinder !== null) return key + prevPathFinder;
        }
    }
    return null
}
